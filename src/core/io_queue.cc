...
public:
    io_desc_read_write(io_queue& ioq, io_queue::priority_class_data& pc, stream_id stream, io_direction_and_length dnl, fair_queue_entry::capacity_t cap, iovec_keeper iovs)
        ...
        , _iovs(std::move(iovs))
    {
        LOGMACRO(io_log, log_level::trace, "dev {} : req {} queue  len {} capacity {}", _ioq.id(), fmt::ptr(this), _dnl.length(), _fq_capacity);
    }

    virtual void set_exception(std::exception_ptr eptr) noexcept override {
        LOGMACRO(io_log, log_level::trace, "dev {} : req {} error", _ioq.id(), fmt::ptr(this));
        _pclass.on_error();
    ...
    virtual void complete(size_t res) noexcept override {
        LOGMACRO(io_log, log_level::trace, "dev {} : req {} complete", _ioq.id(), fmt::ptr(this));
    ...
    void dispatch() noexcept {
        LOGMACRO(io_log, log_level::trace, "dev {} : req {} submit", _ioq.id(), fmt::ptr(this));
        auto now = io_queue::clock_type::now();
