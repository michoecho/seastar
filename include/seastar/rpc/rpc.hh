...
public:
    ...
    // Each RPC connection has one such worker fiber, whose sole purpose
    // is to eventually send an empty RPC frame over the connection whenever
    // the compressor demands it (by signalling a condition variable).
    //
    // This is a mechanism which allows the compressor to communicate
    // something to the other end of the connection (e.g. to re-negotiate
    // some parameters of compression, such as a dictionary).
