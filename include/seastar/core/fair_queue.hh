...
private:
    ...
    // group bucket tail.
    void grab_capacity(const fair_queue_entry& ent) noexcept;
    // Shaves off the fulfilled frontal part from `_pending` (if any),
    // and returns the fulfilled tokens.
    //
    // Assumes that `_group.maybe_replenish_capacity()` was called recently.
    capacity_t reap_pending_capacity() noexcept;
public:
